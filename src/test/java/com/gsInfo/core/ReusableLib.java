...
public abstract class ReusableLib {
    ...
    protected Boolean isDisplayedinPage(By el) {
        try {
        	FluentWait<WebDriver> wait = new WebDriverWait(driver.getWebDriver(), 35)
    ...
    protected void setAttribute(By el,String Value)
    {
    	 WebElement elment=getElement(el);
    	 JavascriptExecutor js = (JavascriptExecutor)driver.getWebDriver();
    	 js.executeScript("arguments[0].value = '" + Value + "'", elment);
    }
    protected void copyPaste(String value,String custmsg)
    {
    	try {
		...
		report.updateTestLog("Copy Paste Data", custmsg, Status.PASS);
    	}catch(Exception e) {
    		report.updateTestLog("Copy Paste Data", "Error in doing copy paste", Status.FAIL);
    	}
    ...
    /**
     * Function to verify if element is clickable or not
     *
     * @return
     */
    protected boolean isElementClickable(By el, int timeWait) {
    	try {
    		FluentWait<WebDriver> wait = new WebDriverWait(driver.getWebDriver(), timeWait)
    				.ignoring(StaleElementReferenceException.class);
    		wait.until(ExpectedConditions.presenceOfElementLocated(el));
    ...
    protected void waitforPageReadyState()
    {
        ...
        try {
        	...
        	report.updateTestLog("Page Load", "Wait until page is loaded", Status.FAIL);
        }
    ...
    /*
     * ******************************************************************* Function
     * Name: findWebElementByName Author : GSInfo Team
     * Purpose : gets list of text from elements
     * ******************************************************************
     */
    protected List<String> xgetWebelementTextList(By el) {
